...
<div class="section">
<p><span class="merged" id="all.27EgHy" title="原文 : If the image to be run has the COHERENCE_HOME environment variable set this will be used to add the following elements to the classpath:">実行するイメージに<code>COHERENCE_HOME</code>環境変数が設定されている場合は、これを使用してクラスパスに次の要素を追加します:</span></p>
...
<li>
<p><span class="merged" id="all.2BF5Zd" title="原文 : /app/resources - the application&rsquo;s other resources"><code>/app/resources</code> - アプリケーションのその他のリソース</span></p>
...
<h4 id="_exclude_the_jib_classpath"><span class="merged" id="all.17NT2M" title="原文 : Exclude the JIB Classpath">JIBクラスパスを除外</span></h4>
<div class="section">
<p><span class="merged" id="all.41MpOa.spl1" title="原文 : If the image is not a JIB image there could be occasions when automatically adding /app/libs/*:/app/classes:/app/resources to the classpath causes issues, for example one or more of those locations exists with files in that should not be on the classpath.">イメージがJIBイメージでない場合、クラスパスに<code>/app/libs/*:/app/classes:/app/resources</code>を自動的に追加すると問題が発生する可能性があります。たとえば、それらのロケーションが1つ以上存在し、ファイルがあるロケーションがクラスパスに存在しない場合などです。</span> <span class="merged" id="all.41MpOa.spl2" title="原文 : In this case the Coherence CRD spec has a field to specify that the JIB classpath should not be used.">この場合、<code>Coherence</code> CRD仕様には、JIBクラスパスを使用しないことを指定するフィールドがあります。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.1AefZS.spl1" title="原文 : If an image will be used that has artifacts in locations other than the defaults discussed above then it is possible to specify additional elements to be added to the classpath.">前述のデフォルト以外のロケーションにアーティファクトがあるイメージを使用する場合は、クラスパスに追加する追加要素を指定できます。</span> <span class="merged" id="all.1AefZS.spl2" title="原文 : The jvm.classpath field in the Coherence CRD spec allows a list of extra classpath values to be provided."><code>Coherence</code> CRD仕様の<code>jvm.classpath</code>フィールドを使用すると、追加のクラスパス値のリストを指定できます。</span> <span class="merged" id="all.1AefZS.spl3" title="原文 : These elements will be added after the JIB classpath described above.">これらの要素は、前述のJIBクラスパスの<em>「後に」</em>追加されます。</span> </p>
...
<ul class="colist">
<li data-value="1"><span class="merged" id="all.1FxRUC.spl1" title="原文 : The classpath field adds /data/lib/* and /data/config to the classpath."><code>classpath</code>フィールドにより、<code>/data/lib/*</code>および<code>/data/config</code>がクラスパスに追加されます。</span> <span class="merged" id="all.1FxRUC.spl2" title="原文 : In an image without the CLASSPATH or COHERENCE_HOME environment variables the effective classpath would be:"><code>CLASSPATH</code>または<code>COHERENCE_HOME</code>環境変数がないイメージでは、有効なクラスパスは次のようになります:</span> </li>
</ul>
<div class="admonition note">
<p class="admonition-inline"><span class="merged" id="all.188Brm.spl1" title="原文 : There is no validation of the elements of the classpath.">クラスパスの要素のバリデーションはありません。</span> <span class="merged" id="all.188Brm.spl2" title="原文 : The elements will not be verified to ensure that the locations exist.">エレメントは、所在地が存在することを確認するために検証されません。</span> <span class="merged" id="all.188Brm.spl3" title="原文 : As long as they are valid values to be used in a JVM classpath they will be accepted.">JVMクラスパスで使用される有効な値であれば、受け入れられます。</span> </p>
</div>
</div>
</div>
...
<div class="section">
<p><span class="merged" id="all.3R3W68.spl1" title="原文 : The Operator supports environment variable expansion in classpath entries.">オペレータは、クラスパス・エントリの環境変数拡張をサポートします。</span> <span class="merged" id="all.3R3W68.spl2" title="原文 : The runner in the Coherence container will replace ${var} or $var in classpath entries with the corresponding environment variable name.">Coherenceコンテナのランナーは、クラスパス・エントリの<code>${var}</code>または<code>$var</code>を対応する環境変数名で置き換えます。</span> </p>

<p><span class="merged" id="all.1VCwQ4" title="原文 : For example if a container has an environment variable of APP_HOME set to /myapp then it could be used in the classpath like this:">たとえば、コンテナに<code>APP_HOME</code>という環境変数が<code>/myapp</code>に設定されている場合、次のようにクラスパスで使用できます:</span></p>
