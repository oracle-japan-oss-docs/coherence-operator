...
<p><span class="merged" id="all.17tAs8.spl1" title="原文 : These examples are not going to cover all the possible use-cases for Extend, the examples are specifically about different ways to connect a client to a Coherence cluster running inside kubernetes.">これらの例は、Extendの考えられるすべてのユースケースを対象としません。具体的には、kubernetes内で動作するCoherenceクラスタにクライアントを接続する様々な方法について例です。</span> <span class="merged" id="all.17tAs8.spl2" title="原文 : Extend is extensively documented in the official Coherence documentation.">拡張については、<a href="https://docs.oracle.com/en/middleware/fusion-middleware/coherence/14.1.2/develop-remote-clients/getting-started-coherenceextend.html" id="" target="_blank" >「公式のCoherenceドキュメント」</a>を参照してください。</span> </p>
