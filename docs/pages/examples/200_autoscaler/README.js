...
<div class="section">
<p><span class="merged" id="all.4dTk9F" title="原文 : Make sure the stable helm repository has been added to Helm if it isn&rsquo;t already present in your local Helm repositories."><code>stable</code> helmリポジトリがローカルのHelmリポジトリに存在しない場合は、Helmに追加されていることを確認します。</span></p>
...
<p><span class="merged" id="all.3i8p9B" title="原文 : Using the HPA to scale as Coherence Pod&rsquo;s heaps become filled is in no way an excuse not to do proper capacity planning and size your Coherence clusters appropriately.">HPAを使用してCoherence Podのヒープがいっぱいになるにつれてスケーリングすることは、適切な容量計画を行わない言い訳でもなく、Coherenceクラスタを適切にサイズ設定してください。</span></p>
