...
<div class="section">
<p><span class="merged" id="all.4N1sW2.spl1" title="原文 : Now we have a &quot;tls&quot; socket provider we can use it to secure Coherence.">これで、"tls"ソケット・プロバイダを使用してCoherenceを保護できるようになりました。</span> <span class="merged" id="all.4N1sW2.spl2" title="原文 : The Coherence documentation has a section on Securing Coherence TCMP with TLS.">Coherenceのドキュメントには、<a href="https://docs.oracle.com/en/middleware/standalone/coherence/14.1.1.2206/secure/using-ssl-secure-communication.html#GUID-21CBAF48-BA78-4373-AC90-BF668CF31776" id="" target="_blank" >「TLSによるCoherence TCMPの保護」</a>に関するセクションがあります。</span> <span class="merged" id="all.4N1sW2.spl3" title="原文 : Securing communication between cluster members is very simple, we just set the coherence.socketprovider system property to the name of the socket provider we want to use.">クラスタ・メンバー間の通信の保護は非常に簡単です。<code>coherence.socketprovider</code>システム・プロパティを、使用するソケット・プロバイダの名前に設定します。</span> <span class="merged" id="all.4N1sW2.spl4" title="原文 : In our case this will be the &quot;tls&quot; provider we configured above, so we would use -Dcoherence.socketprovider=tls">この場合、これは上で構成した"tls"プロバイダであるため、<code>-Dcoherence.socketprovider=tls</code>を使用</span> </p>
...
<div class="section">
<p><span class="merged" id="all.4dZVMg.spl1" title="原文 : A common connection type to secure are client connections into the cluster from Coherence Extend clients.">セキュアにする一般的な接続タイプは、Coherence Extendクライアントからのクラスタへのクライアント接続です。</span> <span class="merged" id="all.4dZVMg.spl2" title="原文 : The Coherence documentation contains details on Using SSL to Secure Extend Client Communication for more in-depth details.">Coherenceドキュメントには、<a href="https://docs.oracle.com/en/middleware/standalone/coherence/14.1.1.2206/secure/using-ssl-secure-communication.html#GUID-0F636928-8731-4228-909C-8B8AB09613DB" id="" target="_blank" >「SSLを使用したクライアント通信の拡張の保護」</a>の詳細が含まれています。</span> </p>
