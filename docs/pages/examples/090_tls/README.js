...
<p><span class="merged" id="all.3fLMNA.spl1" title="原文 : In this example we are going to use Cert Manager to manage the keys and certs for our Coherence server and clients.">この例では、<a href="https://cert-manager.io" id="" target="_blank" >「証明書マネージャ」</a>を使用して、Coherenceサーバーおよびクライアントのキーおよび証明書を管理します。</span> <span class="merged" id="all.3fLMNA.spl2" title="原文 : Cert Manage makes managing certificates in Kubernetes very simple, but it isn&rsquo;t the only solution.">証明書管理により、Kubernetesの証明書の管理が非常に簡単になりますが、唯一の解決策ではありません。</span> </p>
...
<ul class="ulist">
<li>
<p><span class="merged" id="all.4CKimw" title="原文 : Install the Operator"><router-link @click.native="this.scrollFix('#install_operator')" to="#install_operator">オペレータをインストール</router-link></span></p>
...
<div class="section">
<p><span class="merged" id="all.3pSqRZ.spl1" title="原文 : In this example we will use self-signed certs as this makes everything easy to get going.">この例では、すべてが簡単に実行できるように、自己署名証明書を使用します。</span> <span class="merged" id="all.3pSqRZ.spl2" title="原文 : Cert Manager has a number of ways to configure real certificates for production use.">Cert Managerには、本番で使用するために実際の証明書を構成する様々な方法があります。</span> <span class="merged" id="all.3pSqRZ.spl3" title="原文 : Assuming that you&rsquo;ve installed Cert Manager using one of the methods in its Install Guide we can proceed to created all of the required resources."><a href="https://cert-manager.io/docs/installation/" id="" target="_blank" >「インストール・ガイド」</a>のメソッドのいずれかを使用して証明書マネージャをインストールした場合、必要なすべてのリソースの作成に進むことができます。</span> </p>
...
<p><span class="merged" id="all.3i0YJq" title="原文 : If we now look at the client&rsquo;s logs:">クライアントのログを確認する場合:</span></p>
