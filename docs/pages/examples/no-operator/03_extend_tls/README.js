...
<div class="section">
<p><span class="merged" id="all.17nkXn.spl1" title="原文 : The Coherence documentation explains how to Use TLS Secure Communication.">Coherenceドキュメントでは、<a href="https://docs.oracle.com/en/middleware/standalone/coherence/14.1.1.0/secure/using-ssl-secure-communication.html#GUID-90E20139-3945-4993-9048-7FBC93B243A3" id="" target="_blank" >「TLSセキュア通信の使用」</a>の方法について説明します。</span> <span class="merged" id="all.17nkXn.spl2" title="原文 : This example is going to use a standard approach to securing Extend with TLS.">この例では、TLSでExtendを保護するための標準アプローチを使用します。</span> <span class="merged" id="all.17nkXn.spl3" title="原文 : To provide the keystores and credentials the example will make use of Kubernetes Secrets to mount those files as Volumes in the StatefulSet.">キーストアおよび資格証明を提供するために、この例ではKubernetes <code>Secrets</code>を使用して、これらのファイルを<code>StatefulSet</code>に<code>Volumes</code>としてマウントします。</span> <span class="merged" id="all.17nkXn.spl4" title="原文 : This is much more flexible and secure than baking them into an application&rsquo;s code or image.">これは、アプリケーションのコードまたはイメージにベイク処理するよりも、非常に柔軟で安全です。</span> </p>
...
<p><span class="merged" id="all.31ROyj.spl1" title="原文 : The command above will create a Secret named coherence-tls containing the files specified.">前述のコマンドでは、指定したファイルを含む<code>coherence-tls</code>という名前の<code>Secret</code>が作成されます。</span> <span class="merged" id="all.31ROyj.spl2" title="原文 : We can now use the Secret in the cluster&rsquo;s StatefulSet">クラスタの<code>StatefulSet</code>で<code>Secret</code>を使用できるようになりました</span> </p>
