...
<div class="section">
<p><span class="merged" id="all.14fe79.spl1" title="原文 : You can run the Coherence cluster and manage them using the Coherence Operator alongside Istio.">Coherenceクラスタを実行し、<a href="https://istio.io" id="" target="_blank" >Istio</a>とともにCoherence Operatorを使用して管理できます。</span> <span class="merged" id="all.14fe79.spl2" title="原文 : Coherence clusters managed with the Coherence Operator 3.5.1 and later work with Istio 1.9.1 and later out of the box.">Coherence Operator 3.5.1で管理されるCoherenceクラスタは、即時利用可能なIstio 1.9.1以降で動作します。</span> <span class="merged" id="all.14fe79.spl3" title="原文 : Coherence caches can be accessed from outside the Coherence cluster via Coherence*Extend, REST, and other supported Coherence clients.">Coherenceキャッシュには、Coherence*Extend、REST、およびサポートされているその他のCoherenceクライアントを介して、Coherenceクラスタの外部からアクセスできます。</span> <span class="merged" id="all.14fe79.spl4" title="原文 : Using Coherence clusters with Istio does not require the Coherence Operator to also be using Istio (and vice-versa) .">IstioでCoherenceクラスタを使用する場合、Coherence OperatorもIstioを使用する必要はありません(逆も同様)。</span> <span class="merged" id="all.14fe79.spl5" title="原文 : The Coherence Operator can manage Coherence clusters independent of whether those clusters are using Istio or not.">Coherence Operatorは、これらのクラスタがIstioを使用しているかどうかに関係なく、Coherenceクラスタを管理できます。</span> </p>
...
<p><span class="merged" id="all.1PHyC4" title="原文 : You can then install the operator using your preferred method in the Operator Installation Guide.">次に、オペレータ<router-link to="/docs/installation/001_installation">「インストール・ガイド」</router-link>の任意のメソッドを使用してオペレータをインストールできます。</span></p>
