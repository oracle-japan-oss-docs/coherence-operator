...
<li>
<p><span class="merged" id="all.17KIYo" title="原文 : Web Hooks - allow ingress to the Operator&rsquo;s web hook port"><router-link @click.native="this.scrollFix('#webhook')" to="#webhook">「Webフック」</router-link> - オペレータwebフック・ポートへのイングレスを許可</span></p>
...
<div class="section">
<p><span class="merged" id="all.3kTyIh.spl1" title="原文 : Kubernetes does not have a &ldquo;deny all&rdquo; policy, but this can be achieved with a regular network policy that specifies a policyTypes of both &apos;Ingress` and Egress but omits any definitions.">Kubernetesには"deny all"ポリシーはありませんが、これは'Ingress`と<code>Egress</code>の両方の<code>policyTypes</code>を指定する通常のネットワーク・ポリシーで実現できますが、定義は省略できます。</span> <span class="merged" id="all.3kTyIh.spl2" title="原文 : A wild-card podSelector: {} applies the policy to all Pods in the namespace.">ワイルドカード<code>podSelector: {}</code>は、ネームスペース内のすべてのポッドにポリシーを適用します。</span> </p>
...
<p><span class="merged" id="all.Oq4sL.spl1" title="原文 : There are various methods to find the IP address of the API server.">APIサーバーのIPアドレスを見つけるには様々なメソッドがあります。</span> <span class="merged" id="all.Oq4sL.spl2" title="原文 : The exact method required may vary depending on the type of Kubernetes cluster being used, for example a simple development cluster running in KinD on a laptop may differ from a cluster running in a cloud provider&rsquo;s infrastructure.">必要なメソッドは、使用するKubernetesクラスタのタイプによって異なる場合があります。たとえば、ラップトップでKinDで実行されている単純な開発クラスタは、クラウド・プロバイダのインフラストラクチャで実行されているクラスタとは異なる場合があります。</span> </p>

<p><span class="merged" id="all.1Hh4Rh" title="原文 : The common way to find the API server&rsquo;s IP address is to use kubectl cluster-info as follows:">APIサーバーのIPアドレスを見つける一般的な方法は、次のように<code>kubectl cluster-info</code>を使用することです:</span></p>
...
<div class="section">
<p><span class="merged" id="all.1R3D8B.spl1" title="原文 : When a Coherence cluster is deployed, on start-up of a Pod the cluster member will connect to the Operator&rsquo;s REST endpoint to query the site name and rack name, based on the Node the Coherence member is running on.">Coherenceクラスタがデプロイされると、ポッドの起動時にクラスタ・メンバーがオペレータのRESTエンドポイントに接続し、Coherenceメンバーが実行されているノードに基づいてサイト名とラック名をクエリーします。</span> <span class="merged" id="all.1R3D8B.spl2" title="原文 : To allow this to happen the Operator needs to be configured with the relevant ingress policy.">これを実行するには、オペレータを関連するイングレス・ポリシーで構成する必要があります。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.2lZkDM" title="原文 : With all the above policies in place, the Operator is able to work correctly, but if a Coherence resource is now created Kubernetes will be unable to call the Operator&rsquo;s webhook without the correct ingress policy.">前述のポリシーをすべて設定すると、オペレータは正しく動作できますが、<code>Coherence</code>リソースが作成された場合、Kubernetesは、正しいイングレス・ポリシーなしでオペレータのwebフックをコールできません。</span></p>
...
<div class="section">
<p><span class="merged" id="all.1ZpdVt.spl1" title="原文 : All Pods in a Coherence cluster must be able to talk to each other (otherwise they wouldn&rsquo;t be a cluster).">Coherenceクラスタ内のすべてのポッドは、相互に通信できる必要があります(そうしないとクラスタになりません)。</span> <span class="merged" id="all.1ZpdVt.spl2" title="原文 : This means that there needs to be ingress and egress policies to allow this.">つまり、これを許可するにはイングレスおよびエグレス・ポリシーが必要です。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.3o31Pe.spl1" title="原文 : If Coherence metrics is enabled there will need to be an ingress policy to allow connections from metrics clients.">Coherenceメトリクスが有効になっている場合、メトリクス・クライアントからの接続を許可するイングレス・ポリシーが必要です。</span> <span class="merged" id="all.3o31Pe.spl2" title="原文 : There would also need to be a similar egress policy in the metrics client&rsquo;s namespace to allow it to access the Coherence metrics endpoints.">また、メトリクス・クライアントのネームスペースには、Coherenceメトリクス・エンドポイントにアクセスできるように、同様のエグレス・ポリシーが必要です。</span> </p>
