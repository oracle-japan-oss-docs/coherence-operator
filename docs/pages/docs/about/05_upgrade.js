...
<p><span class="merged" id="all.3iF6zN" title="原文 : See the Applications section of the doecumentation for more details.">詳細は、説明の<router-link to="/docs/applications/010_overview">「アプリケーション」</router-link>セクションを参照してください。</span></p>
...
<div class="section">
<p><span class="merged" id="all.4SopQE.spl1" title="原文 : A lot of the fields in the Coherence CRD are the same as when defining a role in version 2."><code>Coherence</code> CRDのフィールドの多くは、バージョン2でロールを定義する場合と同じです。</span> <span class="merged" id="all.4SopQE.spl2" title="原文 : Whilst a number of new fields and features have been added in version 3, a handful of fields have moved, and a small number, that no longer made sense, have been removed.">バージョン3で多数の新しいフィールドや機能が追加されている間に、いくつかのフィールドが移動し、意味のない数値が少なくなりました。</span> <span class="merged" id="all.4SopQE.spl3" title="原文 : The Coherence Spec page documents the full Coherence CRD, so it is simple to locate where a field might have moved to."><router-link to="/docs/about/04_coherence_spec">「Coherence仕様」</router-link>ページでは、完全な<code>Coherence</code> CRDがドキュメント化されるため、フィールドの移動先の場所を簡単に特定できます。</span> </p>

</div>

<h2 id="_logging_and_fluentd"><span class="merged" id="all.1vffzo" title="原文 : Logging and Fluentd">ロギングおよびFluentd</span></h2>
<div class="section">
<p><span class="merged" id="all.44HlLG.spl1" title="原文 : Version 3 of the operator no longer has fields to configure a Fluentd side-car container.">オペレータのバージョン3には、Fluentdサイドカー・コンテナを構成するためのフィールドがありません。</span> <span class="merged" id="all.44HlLG.spl2" title="原文 : There are a lot of different ways to configure Fluentd and making the Operator accomodate all of these was becoming too much of a head-ache to do in a backwards compatible way.">Fluentdを構成し、オペレータに納めるためのさまざまな方法があります。これらはすべて、後方互換性のある方法で行う頭痛が多すぎます。</span> <span class="merged" id="all.44HlLG.spl3" title="原文 : If a Fluentd side-car is required it can just be added to the Coherence resource spec as an additional container, so there is no limitation on the Fluentd configuration.">Fluentdサイドカーが必要な場合、<code>Coherence</code>リソース仕様に追加コンテナとしてのみ追加できるため、Fluentd構成に制限はありません。</span> <span class="merged" id="all.44HlLG.spl4" title="原文 : See the Logging documentation for more examples.">詳細は、<router-link to="/docs/logging/010_overview">「ロギング・ドキュメント」</router-link>を参照してください。</span> </p>
