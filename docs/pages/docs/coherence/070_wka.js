...
<div class="section">
<p><span class="merged" id="all.33N08z.spl1" title="原文 : A Coherence cluster is made up of one or more JVMs.">Coherenceクラスタは、1つ以上のJVMで構成されます。</span> <span class="merged" id="all.33N08z.spl2" title="原文 : In order for these JVMs to form a cluster they need to be able to discover other cluster members.">これらのJVMがクラスタを形成するには、他のクラスタ・メンバーを検出できる必要があります。</span> <span class="merged" id="all.33N08z.spl3" title="原文 : The default mechanism for discovery is multicast broadcast but this does not work in most container environments.">検出のデフォルトのメカニズムはマルチキャスト・ブロードキャストですが、これはほとんどのコンテナ環境では機能しません。</span> <span class="merged" id="all.33N08z.spl4" title="原文 : Coherence provides an alternative mechanism where the addresses of the hosts where the members of the cluster will run is provided in the form of a &quot;well known address&quot; (or WKA) list.">Coherenceは、クラスタのメンバーが実行されるホストのアドレスが<a href="https://docs.oracle.com/en/middleware/standalone/coherence/14.1.1.0/develop-applications/setting-cluster.html#GUID-E8CC7C9A-5739-4D12-B88E-A3575F20D63B" id="" target="_blank" >「既知のアドレス」(またはWKA)リスト」</a>の形式で提供される代替メカニズムを提供します。</span> <span class="merged" id="all.33N08z.spl5" title="原文 : This address list is then used by Coherence when it starts in a JVM to discover other cluster members running on the hosts in the WKA list.">このアドレス・リストは、WKAリスト内のホストで実行されている他のクラスタ・メンバーを検出するためにJVMで起動するときに、Coherenceによって使用されます。</span> </p>
...
<p><span class="merged" id="all.4biOea" title="原文 : A role can be excluded from the WKA list by setting the excludeFromWKA field of the coherence section of the deployment&rsquo;s spec to true.">デプロイメントの仕様の<code>coherence</code>セクションの<code>excludeFromWKA</code>フィールドを<code>true</code>に設定すると、WKAリストからロールを除外できます。</span></p>
...
<div class="admonition warning">
<p class="admonition-inline"><span class="merged" id="all.27a5af.spl1" title="原文 : When excluding a deployment from WKA it is important that at least one deployment that is part of the WKA list has been started first otherwise the non-WKA role members cannot start.Eventually the K8s readiness probe for these Pods would time-out causing K8s to restart them but this would not be a desirable way to start a cluster.">WKAからデプロイメントを除外する場合は、WKAリストの一部である少なくとも1つのデプロイメントが最初に起動されていることが重要です。そうしないと、WKA以外のロール・メンバーがstart.Eventuallyを実行できず、K8sレディネス・プローブでこれらのポッドを再起動するとタイムアウトになりますが、これはクラスタを起動する望ましい方法ではありません。</span> <span class="merged" id="all.27a5af.spl2" title="原文 : The start-up order can be controlled by configuring the deployment&rsquo;s startQuorum list, as described in the documentation section on deployment start-up ordering.">起動順序は、<router-link to="/docs/ordering/010_overview">「デプロイメント開始順序」</router-link>のドキュメント・セクションの説明に従って、デプロイメントの<code>startQuorum</code>リストを構成することで制御できます。</span> </p>
</div>
</div>
