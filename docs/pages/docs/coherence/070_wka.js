...
<div class="section">
<p><span class="merged" id="all.TM2pL.spl1" title="原文 : A Coherence cluster is made up of one or more JVMs.">Coherenceクラスタは、1つ以上のJVMで構成されます。</span> <span class="merged" id="all.TM2pL.spl2" title="原文 : In order for these JVMs to form a cluster they need to be able to discover other cluster members.">これらのJVMがクラスタを形成するには、他のクラスタ・メンバーを検出できる必要があります。</span> <span class="merged" id="all.TM2pL.spl3" title="原文 : The default mechanism for discovery is multicast broadcast but this does not work in most container environments.">検出のデフォルトのメカニズムはマルチキャスト・ブロードキャストですが、これはほとんどのコンテナ環境では機能しません。</span> <span class="merged" id="all.TM2pL.spl4" title="原文 : Coherence provides an alternative mechanism where the addresses of the hosts where the members of the cluster will run is provided in the form of a &quot;well known address&quot; (or WKA) list.">Coherenceは、クラスタのメンバーが実行されるホストのアドレスが<a href="https://docs.oracle.com/en/middleware/fusion-middleware/coherence/14.1.2/develop-applications/setting-cluster.html#GUID-E8CC7C9A-5739-4D12-B88E-A3575F20D63B" id="" target="_blank" >「既知のアドレス」(またはWKA)リスト」</a>の形式で提供される代替メカニズムを提供します。</span> <span class="merged" id="all.TM2pL.spl5" title="原文 : This address list is then used by Coherence when it starts in a JVM to discover other cluster members running on the hosts in the WKA list.">このアドレス・リストは、WKAリスト内のホストで実行されている他のクラスタ・メンバーを検出するためにJVMで起動するときに、Coherenceによって使用されます。</span> </p>
