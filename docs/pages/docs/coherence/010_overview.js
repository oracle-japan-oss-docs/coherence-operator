...
<p class="admonition-inline"><span class="merged" id="all.2Q3ckK.spl1" title="原文 : The Coherence settings in the Coherence CRD spec typically set system property values that will be passed through to the Coherence JVM command line, which in turn configure Coherence."><code>Coherence</code> CRD仕様のCoherence設定では、通常、Coherence JVMコマンドラインに渡されるシステム・プロパティ値が設定され、これによってCoherenceが構成されます。</span> <span class="merged" id="all.2Q3ckK.spl2" title="原文 : This is the same behaviour that would occur when running Coherence outside of containers.">これは、コンテナの外部でCoherenceを実行する場合に発生する動作と同じです。</span> <span class="merged" id="all.2Q3ckK.spl3" title="原文 : Whether these system properties actually apply or not depends on the application code.">これらのシステム・プロパティが実際に適用されるかどうかは、アプリケーション・コードによって異なります。</span> <span class="merged" id="all.2Q3ckK.spl4" title="原文 : For example, it is simple to override the Coherence operational configuration file in a jar file deployed as part of an application’s image in such a way that will cause all the normal Coherence system properties to be ignored.">たとえば、アプリケーションのイメージの一部としてデプロイされたjarファイル内のCoherence操作構成ファイルをオーバーライドすると、通常のCoherenceシステム・プロパティがすべて無視されるようになります。</span> <span class="merged" id="all.2Q3ckK.spl5" title="原文 : If that is done then the Coherence settings discussed in this documentation will not apply.">これを実行した場合、このドキュメントで説明するCoherence設定は適用されません。</span> <span class="merged" id="all.2Q3ckK.spl6" title="原文 :  For example, adding a tangosol-coherence-override.xml file to a jar on the application’s classpath that contains an overridden &lt;configurable-cache-factory-config&gt; section with a hard coded cache configuration file name would mean that the Coherence CRD spec.coherence.cacheConfig field, that sets the coherence.cacheconfig system property, would be ignored."><br>たとえば、ハード・コード・キャッシュ構成ファイル名を持つオーバーライドされた<code>&lt;configurable-cache-factory-config></code>セクションを含むアプリケーション・クラスパスのjarに<code>tangosol-coherence-override.xml</code>ファイルを追加すると、<code>coherence.cacheconfig</code>システム・プロパティを設定する<code>Coherence</code> CRD <code>spec.coherence.cacheConfig</code>フィールドは無視されます。</span> <span class="merged" id="all.2Q3ckK.spl7" title="原文 :  It is, therefore, entirely at the application developer’s discretion whether they use the fields of the Coherence CRD to configure Coherence, or they put those settings into configuration files, either hard coded into jar files or picked up at runtime from files mapped from Kubernetes volumes, config maps, secrets, etc."><br>したがって、アプリケーション開発者は、<code>Coherence</code> CRDのフィールドを使用してCoherenceを構成するか、これらの設定をJARファイルにハードコードするか、Kubernetesボリューム、構成マップ、シークレットなどからマップされたファイルから実行時にピック・アップするかに関係なく、完全にアプリケーション開発者の判断で判断します。</span> </p>
</div>
</div>
</doc-view>
