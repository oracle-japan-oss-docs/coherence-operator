...
<div class="section">
<p><span class="merged" id="all.3V6OsE.spl1" title="原文 : The StatusHA check performed by the Operator uses a http endpoint that the Operator runs on a well-known port in the Coherence JVM.">オペレータによって実行されるStatusHAチェックは、オペレータがCoherence JVMの既知のポートで実行するhttpエンドポイントを使用します。</span> <span class="merged" id="all.3V6OsE.spl2" title="原文 : This endpoint performs a simple check to verify that none of the partitioned cache services known about by Coherence have an endangered status.">このエンドポイントは、Coherenceが認識するパーティション化されたキャッシュ・サービスのいずれにも、危険にさらされたステータスがないことを確認するための簡単なチェックを実行します。</span> <span class="merged" id="all.3V6OsE.spl3" title="原文 : If an application has a different concept of what &quot;safe&quot; means it can implement a different method to check the status during scaling.">アプリケーションに「安全」とは別の概念がある場合、スケーリング中にステータスをチェックするために別のメソッドを実装できます。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.2nJP1x" title="原文 : An exec probe works the same way as a Kubernetes Exec liveness probe">execプローブは<a href="https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/#define-a-liveness-command" id="" target="_blank" >「Kubernetesリブネス・プローブ」</a>と同じように動作</span></p>
