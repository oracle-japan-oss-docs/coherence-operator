...
<p><span class="merged" id="all.4J7Gdi.spl1" title="原文 : The commands below will download and run the shell script to import the dashboards.">次のコマンドは、ダッシュボードをインポートするためにシェル・スクリプトをダウンロードして実行します。</span> <span class="merged" id="all.4J7Gdi.spl2" title="原文 : Change the &lt;GRAFANA-USER&gt; and &lt;GRAFANA_PWD&gt; to the Grafana credentials for your environment."><code>&lt;GRAFANA-USER></code>および<code>&lt;GRAFANA_PWD></code>を環境のGrafana資格証明に変更します。</span> <span class="merged" id="all.4J7Gdi.spl3" title="原文 : For example if using the default Prometheus Operator installation they are as specified on the Access Grafana section of the Quick Start page.">たとえば、デフォルトのPrometheusオペレータ・インストールを使用する場合、<a href="https://prometheus-operator.dev/docs/prologue/quick-start/#access-grafana" id="" target="_blank" >「クイック・スタートのGrafanaセクションにアクセス」</a>ページで指定されているようになります。</span> <span class="merged" id="all.4J7Gdi.spl4" title="原文 : We do not document the credentials here as the default values have been known to change between Prometheus Operator and Grafana versions.">PrometheusオペレータとGrafanaバージョン間で変更することがわかっているため、ここで資格証明を文書化しません。</span> </p>
