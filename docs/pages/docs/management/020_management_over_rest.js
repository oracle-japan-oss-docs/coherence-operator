...
<div class="section">
<p><span class="merged" id="all.2StENU" title="原文 : Since version 12.2.1.4 Coherence has had functionality to expose a management API over REST.">バージョン12.2.1.4から、CoherenceにはRESTを介して管理APIを公開する機能がありました。</span></p>
...
<p><span class="merged" id="all.12pNpH" title="原文 : The example below shows how to enable and access Coherence MBeans using Management over REST.">次の例は、RESTの管理を使用してCoherence MBeansを有効にし、アクセスする方法を示しています。</span></p>
...
<h3 id="_deploy_coherence_with_management_over_rest_enabled"><span class="merged" id="all.2USgMY" title="原文 : Deploy Coherence with Management over REST Enabled">RESTを介した管理でのCoherenceのデプロイの有効化</span></h3>
<div class="section">
<p><span class="merged" id="all.2bEW3F" title="原文 : To deploy a Coherence resource with management over REST enabled and exposed on a port, the simplest yaml would look like this:">管理を持つ<code>Coherence</code>リソースをREST上で有効化してポート上で公開するには、最も単純なyamlは次のようになります:</span></p>
...
<ul class="colist">
<li data-value="1"><span class="merged" id="all.3FP4g3" title="原文 : Setting the coherence.management.enabled field to true will enable Management over REST"><code>coherence.management.enabled</code>フィールドを<code>true</code>に設定すると、RESTの管理が有効になります</span></li>
<li data-value="2"><span class="merged" id="all.1M78av.spl1" title="原文 : To expose Management over REST via a Service it is added to the ports list."><code>Service</code>を介してRESTを介して管理を公開するには、<code>ports</code>リストに追加されます。</span> <span class="merged" id="all.1M78av.spl2" title="原文 : The management port is a special case where the port number is optional so in this case Management over REST will bind to the default port 30000. (see Exposing Ports for details)"><code>management</code>ポートは、<code>port</code>番号がオプションであるため、この場合、Management over RESTはデフォルトのポート<code>30000</code>にバインドされます(詳細は、<router-link to="/docs/ports/020_container_ports">「ポートの公開」</router-link>を参照)</span> </li>
</ul>
<p><span class="merged" id="all.vQ7vZ" title="原文 : To expose Management over REST on a different port the alternative port value can be set in the coherence.management section, for example:">別のポートでRESTを介して管理を公開するには、<code>coherence.management</code>セクションで代替ポート値を設定できます。次に例を示します:</span></p>
...
<ul class="colist">
<li data-value="1"><span class="merged" id="all.3argMW" title="原文 : Management over REST will now be exposed on port 8080">RESTでの管理がポート<code>8080</code>で公開されるようになりました</span></li>
</ul>
</div>
...
<div class="section">
<p><span class="merged" id="all.RRdmd" title="原文 : After installing the basic management-cluster.yaml from the first example above there would be a three member Coherence cluster installed into Kubernetes.">上の最初の例から基本的な<code>management-cluster.yaml</code>をインストールすると、3つのメンバーCoherenceクラスタがKubernetesにインストールされます。</span></p>
...
<p><span class="merged" id="all.2Dp1Jz.spl1" title="原文 : In a test or development environment the simplest way to reach an exposed port is to use the kubectl port-forward command.">テストまたは開発環境では、公開されたポートに到達する最も簡単な方法は、<code>kubectl port-forward</code>コマンドを使用することです。</span> <span class="merged" id="all.2Dp1Jz.spl2" title="原文 : For example to connect to the first Pod in the deployment:">たとえば、デプロイメントの最初の<code>Pod</code>に接続するには:</span> </p>
...
<div class="section">
<p><span class="merged" id="all.2SnRCd" title="原文 : Now that a port is being forwarded from localhost to a Pod in the cluster the Management over REST endpoints can be accessed.">これで、ポートがローカル・ホストからクラスタ内の<code>Pod</code>に転送されるため、RESTエンドポイントを介した管理にアクセスできます。</span></p>
...
<p><span class="merged" id="all.2rLj0Z" title="原文 : Which should result in a response similar to the following:">次のようなレスポンスが発生します:</span></p>
...
<p><span class="merged" id="all.3Vpw6j" title="原文 : Which should result in a response like the following:">次のようなレスポンスが発生します:</span></p>
...
<div class="admonition note">
<p class="admonition-inline"><span class="merged" id="all.4MQfw" title="原文 : The above output has been truncated due to the large size.">上記の出力はサイズが大きいため切り捨てられました。</span></p>
</div>
</div>
...
<li>
<p><span class="merged" id="all.3BwFVO" title="原文 : Enabling SSL"><router-link @click.native="this.scrollFix('#docs/clusters/058_coherence_management.adoc')" to="#docs/clusters/058_coherence_management.adoc">SSLの有効化</router-link></span></p>
