...
<li data-value="2"><span class="merged" id="all.3Kvrbg" title="原文 : The port field in the Coherence CRD&rsquo;s port spec maps to the containerPort in the Container port spec."><code>Coherence</code> CRDのポート仕様の<code>port</code>フィールドは、コンテナ・ポート仕様の<code>containerPort</code>にマップされます。</span></li>
</ul>
...
<div class="section">
<p><span class="merged" id="all.2z44wn.spl1" title="原文 : Exposing the Coherence metrics port or Coherence Management over REST port are treated as a special case in the configuration.">RESTポートを介したCoherenceメトリクス・ポートまたはCoherence管理の公開は、構成の特殊なケースとして扱われます。</span> <span class="merged" id="all.2z44wn.spl2" title="原文 : Normally both the port&rsquo;s name and port value are required fields.">通常、ポートの<code>name</code>値と<code>port</code>値の両方が必須フィールドです。</span> <span class="merged" id="all.2z44wn.spl3" title="原文 : If the port name is metrics or management the Operator already knows the port values (either from the defaults or from the metrics or management configuration) so these do not need to be specified again.">ポート名が<code>metrics</code>または<code>management</code>の場合、オペレータはすでに<code>port</code>値(デフォルトまたはメトリクスまたは管理構成のいずれか)を認識しているため、これらを再度指定する必要はありません。</span> </p>
