...
<p><span class="merged" id="all.hyOeP" title="原文 : If the configuration of Services for ports provided by the Coherence CRD spec is not sufficient or cannot provide the required Service configuration then it is always possible to just create your own Services in Kubernetes."><code>Coherence</code> CRD仕様で提供されるポートの<code>Services</code>の構成が不十分であるか、必要な<code>Service</code>構成を提供できない場合、常にKubernetesに独自の<code>Services</code>を作成できます。</span></p>
...
<p></p>
<p><span class="merged" id="all.dlVEr" title="原文 : Adding additional container ports to the Coherence container.">Coherenceコンテナに追加のコンテナ・ポートを追加します。</span></p>
</v-card-text> </v-card> </v-flex> <v-flex xs12 sm4 lg3> <v-card> <router-link to="/docs/ports/030_services"><div class="card__link-hover"/>
</router-link> <v-card-title primary class="headline layout justify-center"><span style="text-align:center">サービスを介したポートの公開</span></v-card-title> <v-card-text class="caption">
<p></p>
<p><span class="merged" id="all.1TIXC8" title="原文 : Configuring Services used to expose ports.">ポートの公開に使用するサービスの構成。</span></p>
...
<p></p>
<p><span class="merged" id="all.2wN6WN" title="原文 : Adding Prometheus ServiceMonitors to expose ports to be scraped for metrics.">Prometheus ServiceMonitorsを追加して、メトリクスでスクレイプされるポートを公開します。</span></p>
</v-card-text>
</v-card>
</v-flex>
</v-layout>
