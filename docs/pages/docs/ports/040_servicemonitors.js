...
<div class="section">
<p><span class="merged" id="all.1QbShm.spl1" title="原文 : When a port exposed on a container is to be used to serve metrics to Prometheus this often requires the addition of a Prometheus ServiceMonitor resource.">コンテナで公開されたポートを使用してPrometheusのメトリクスを提供する場合、多くの場合、Prometheus <code>ServiceMonitor</code>リソースを追加する必要があります。</span> <span class="merged" id="all.1QbShm.spl2" title="原文 : The Coherence Operator makes it simple to add a ServiceMonitor for an exposed port.">Coherence Operatorを使用すると、公開されたポートの<code>ServiceMonitor</code>を簡単に追加できます。</span> <span class="merged" id="all.1QbShm.spl3" title="原文 : The advantage of specifying the ServiceMonitor configuration in the Coherence CRD spec is that the ServiceMonitor resource will be created, updated and deleted as part of the lifecycle of the Coherence resource, and does not need to be managed separately."><code>Coherence</code> CRD仕様で<code>ServiceMonitor</code>構成を指定する利点は、<code>ServiceMonitor</code>リソースが<code>Coherence</code>リソースのライフサイクルの一部として作成、更新および削除され、個別に管理する必要がないことです。</span> </p>
...
<p><span class="merged" id="all.21KblV" title="原文 : The ServiceMonitor created from the spec above will look like this: For example:">前述の仕様から作成された<code>ServiceMonitor</code>は次のようになります: たとえば:</span></p>
