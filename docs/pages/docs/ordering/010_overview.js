...
<div class="section">
<p><span class="merged" id="all.1kCZh6.spl1" title="原文 : The default behaviour of the operator is to create the StatefulSet for a Coherence deployment immediately.">オペレータのデフォルトの動作では、<code>Coherence</code>デプロイメントの<code>StatefulSet</code>を即時に作成します。</span> <span class="merged" id="all.1kCZh6.spl2" title="原文 : Sometimes this behaviour is not suitable if, for example, when application code running in one deployment depends on the availability of another deployment.">たとえば、あるデプロイメントで実行されているアプリケーション・コードが別のデプロイメントの可用性に依存する場合など、この動作が適していない場合があります。</span> <span class="merged" id="all.1kCZh6.spl3" title="原文 : Typically, this might be storage disabled members having functionality that relies on the storage members being ready first.">通常、これはストレージが無効なメンバーで、最初に準備されているストレージ・メンバーに依存する機能を持つ可能性があります。</span> <span class="merged" id="all.1kCZh6.spl4" title="原文 : The Coherence CRD allows can be configured with a startQuorum that defines a deployment&rsquo;s dependency on other deployments in the cluster."><code>Coherence</code> CRDでは、クラスタ内の他のデプロイメントへのデプロイメントの依存関係を定義する<code>startQuorum</code>を使用して構成できます。</span> </p>
...
<ul class="colist">
<li data-value="1"><span class="merged" id="all.2CsHl5" title="原文 : The data deployment does not specify a startQuorum so this deployment&rsquo;s StatefulSet will be created immediately by the operator."><code>data</code>デプロイメントでは<code>startQuorum</code>を指定しないため、このデプロイメントの<code>StatefulSet</code>はオペレータによってすぐに作成されます。</span></li>
