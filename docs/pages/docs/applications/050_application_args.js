...
<div class="section">
<p><span class="merged" id="all.2pdTNG.spl1" title="原文 : When running a custom application there may be a requirement to pass arguments to the application&rsquo;s main class.">カスタム・アプリケーションを実行するときに、アプリケーションのメイン・クラスに引数を渡す必要がある場合があります。</span> <span class="merged" id="all.2pdTNG.spl2" title="原文 : By default, there are no application arguments but any arguments required can be specified in the application.args list in the Coherence resource spec.">デフォルトでは、アプリケーション引数はありませんが、必要な引数は<code>Coherence</code>リソース仕様の<code>application.args</code>リストで指定できます。</span> </p>
