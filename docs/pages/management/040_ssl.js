...
<h2 id="_ssl_with_management_over_rest"><span class="merged" id="all.4Li805" title="原文 : SSL with Management over REST">RESTを介した管理でのSSL</span></h2>
<div class="section">
...
<ul class="colist">
<li data-value="1"><span class="merged" id="all.FuYcg" title="原文 : The enabled field when set to true enables SSL for metrics or when set to false disables SSL"><code>true</code>に設定されている場合、<code>enabled</code>フィールドは、メトリクスに対してSSLを有効にするか、<code>false</code>に設定するとSSLが無効になります</span></li>
<li data-value="2"><span class="merged" id="all.2kW9G2" title="原文 : The keyStore field sets the name of the Java key store file that should be used to obtain the server&rsquo;s key"><code>keyStore</code>フィールドは、サーバーのキーの取得に使用するJavaキー・ストア・ファイルの名前を設定</span></li>
<li data-value="3"><span class="merged" id="all.N3g8j" title="原文 : The optional keyStoreType field sets the type of the key store file, the default value is JKS">オプションの<code>keyStoreType</code>フィールドはキー・ストア・ファイルのタイプを設定します。デフォルト値は<code>JKS</code>です</span></li>
...
<li data-value="7"><span class="merged" id="all.R70o2" title="原文 : The optional keyStoreAlgorithm sets the algorithm name for the key store, the default value is SunX509">オプションの<code>keyStoreAlgorithm</code>は、キー・ストアのアルゴリズム名を設定します。デフォルト値は<code>SunX509</code>です</span></li>
<li data-value="8"><span class="merged" id="all.yx8Wj" title="原文 : The trustStore field sets the name of the Java trust store file that should be used to obtain the server&rsquo;s key"><code>trustStore</code>フィールドは、サーバーのキーの取得に使用するJavaトラスト・ストア・ファイルの名前を設定</span></li>
...
<li data-value="13"><span class="merged" id="all.Fa8yb" title="原文 : The optional requireClientCert field if set to true enables two-way SSL where the client must also provide a valid certificate"><code>true</code>に設定されている場合、オプションの<code>requireClientCert</code>フィールドにより双方向SSLが有効になり、クライアントは有効な証明書も指定する必要があります</span></li>
...
<p><span class="merged" id="all.cIN6K.spl1" title="原文 : The various files and keystores referred to in the configuration above can be any location accessible in the image used by the coherence container in the deployment&rsquo;s Pods.">上の構成で参照されている様々なファイルおよびキーストアは、デプロイメントの<code>Pods</code>の<code>coherence</code>コンテナで使用されるイメージでアクセス可能な任意のロケーションにできます。</span> <span class="merged" id="all.cIN6K.spl2" title="原文 : Typically, for things such as SSL keys and certs, these would be provided by obtained from Secrets loaded as additional Pod Volumes.">通常、SSLキーや証明書などのものについては、追加の<code>Pod</code> <code>Volumes</code>としてロードされた<code>Secrets</code>から取得して提供されます。</span> <span class="merged" id="all.cIN6K.spl3" title="原文 : See Add Secrets Volumes for the documentation on how to specify secrets as additional volumes.">追加ボリュームとしてシークレットを指定する方法については、<router-link to="/other/060_secret_volumes">「Secretsボリュームの追加」</router-link>を参照してください。</span> </p>
