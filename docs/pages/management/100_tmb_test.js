...
<div class="section">
<p><span class="merged" id="all.486GBH.spl1" title="原文 : Coherence provides utilities that can be used to test network performance, which obviously has a big impact on a distributed system such as Coherence.">Coherenceは、ネットワーク・パフォーマンスをテストするために使用できるユーティリティを提供します。これは、Coherenceなどの分散システムに大きな影響を与えます。</span> <span class="merged" id="all.486GBH.spl2" title="原文 : The documentation for these utilities can be found in the official Coherence Documentation.">これらのユーティリティのドキュメントは、正式な<a href="https://docs.oracle.com/en/middleware/standalone/coherence/14.1.1.0/administer/performing-network-performance-test.html#GUID-7267AB06-6353-416E-B9FD-A75F7FBFE523" id="" target="_blank" >「Coherenceドキュメント」</a>にあります。</span> </p>

<p><span class="merged" id="all.4AqR2s.spl1" title="原文 : Whilst generally these tests would be run on server hardware, with more and more Coherence deployments moving into the cloud and into Kubernetes these tests can also be performed in Pods to measure inter-Pod network performance.">通常、これらのテストはサーバー・ハードウェアで実行され、より多く、クラウドに移行するCoherenceデプロイメントおよびKubernetesで、これらのテストは、Pod間のネットワーク・パフォーマンスを測定するために<code>Pods</code>で実行することもできます。</span> <span class="merged" id="all.4AqR2s.spl2" title="原文 : This test can be used to see the impact of running Pods across different zones, or on different types of Kubernetes networks, with different Pod resource settings, etc.">このテストは、異なるゾーン間での<code>Pods</code>の実行、または異なるタイプのKubernetesネットワーク(<code>Pod</code>リソース設定など)への影響を確認するために使用できます。</span> </p>

</div>

<h2 id="_run_the_message_bus_test_in_pods"><span class="merged" id="all.39quIw" title="原文 : Run the Message Bus Test in Pods">ポッドでのメッセージ・バス・テストの実行</span></h2>
<div class="section">
<p><span class="merged" id="all.4aY7tv.spl1" title="原文 : The message bus test can easily be run using Pods in Kubernetes.">メッセージ・バス・テストは、Kubernetesの<code>Pods</code>を使用して簡単に実行できます。</span> <span class="merged" id="all.4aY7tv.spl2" title="原文 : Using the example from the Coherence documentation there will need to be two Pods, a listener and a sender.">Coherenceのドキュメントの例では、2つの<code>Pods</code>、リスナーおよび送信者が必要です。</span> <span class="merged" id="all.4aY7tv.spl3" title="原文 : This example will create a Service for the listener so that the sender Pod can use the Service name to resolve the listener Pod address.">この例では、送信者の<code>Pod</code>が<code>Service</code>名を使用してリスナー<code>Pod</code>アドレスを解決できるように、リスナーの<code>Service</code>を作成します。</span> </p>
...
<ul class="colist">
<li data-value="1"><span class="merged" id="all.2wTDUd" title="原文 : This example uses the Coherence image from Oracle container registry, but any image with coherence.jar in it could be used.">この例では、Oracleコンテナ・レジストリのCoherenceイメージを使用しますが、<code>coherence.jar</code>を含むイメージを使用できます。</span></li>
<li data-value="2"><span class="merged" id="all.3HDxWY" title="原文 : The command line that the container will execute is exactly the same as that for the listener process in the Coherence Documentation.">コンテナが実行するコマンドラインは、<a href="https://docs.oracle.com/en/middleware/standalone/coherence/14.1.1.0/administer/performing-network-performance-test.html#GUID-7267AB06-6353-416E-B9FD-A75F7FBFE523" id="" target="_blank" >「Coherenceドキュメント」</a>のリスナー・プロセスの場合とまったく同じです。</span></li>
</ul>
...
<p><span class="merged" id="all.SJLM8" title="原文 : Retrieving the logs for the listener Pod the messages should show that the Pod has started:">リスナー<code>Pod</code>のログを取得すると、<code>Pod</code>が開始したことを示すメッセージが表示されます:</span></p>
...
<ul class="colist">
<li data-value="1"><span class="merged" id="all.23HKBi" title="原文 : Again, the command line is the same as that for the sender process in the Coherence Documentation.">この場合も、コマンドラインは、<a href="https://docs.oracle.com/en/middleware/standalone/coherence/14.1.1.0/administer/performing-network-performance-test.html#GUID-7267AB06-6353-416E-B9FD-A75F7FBFE523" id="" target="_blank" >「Coherenceドキュメント」</a>の送信者プロセスの場合と同じです。</span></li>
...
<p><span class="merged" id="all.3c5941" title="原文 : Retrieving the logs for the sender Pod the messages should show that the Pod has started and show the test results:">送信者<code>Pod</code>のログを取得すると、<code>Pod</code>が開始されたことを示すメッセージが表示され、テスト結果が表示されます:</span></p>
...
<div class="section">
<p><span class="merged" id="all.3clR3v.spl1" title="原文 : In the example above the Pods will be scheduled wherever Kubernetes decides to put them.">上の例では、<code>Pods</code>は、配置を決定したKubernetesであればどこでもスケジュールされます。</span> <span class="merged" id="all.3clR3v.spl2" title="原文 : This could have a big impact on the test result for different test runs.">これは、異なるテスト実行のテスト結果に大きな影響を与える可能性があります。</span> <span class="merged" id="all.3clR3v.spl3" title="原文 : For example in a Kubernetes cluster that spans zones and data centres, if the two Pods get scheduled in different data centres this will have worse results than if the two Pods get scheduled onto the same node.">たとえば、ゾーンおよびデータ・センターにまたがるKubernetesクラスタでは、2つの<code>Pods</code>が異なるデータ・センターでスケジュールされる場合、2つの<code>Pods</code>が同じノードにスケジュールされている場合よりも、この結果はより悪くなります。</span> </p>

<p><span class="merged" id="all.2KnJyy" title="原文 : To get consistent results add node selectors, taints, tolerations etc, as covered in the Kubernetes assign Pods to Nodes documentation.">一貫性のある結果を得るには、Kubernetes <a href="https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/" id="" target="_blank" >「ノードへのポッドの割当て」</a>ドキュメントの説明に従って、ノード・セレクタ、Taints、Tolerationsなどを追加します。</span></p>
