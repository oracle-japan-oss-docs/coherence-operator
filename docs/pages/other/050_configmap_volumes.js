...
<dd slot="desc"><p><span class="merged" id="all.3kwkrC" title="原文 : Additional Volumes and VolumeMounts from ConfigMaps can easily be added to a Coherence resource."><code>ConfigMaps</code>からの追加の<code>Volumes</code>および<code>VolumeMounts</code>は、<code>Coherence</code>リソースに簡単に追加できます。</span></p>
</dd>
</dl>
</v-card-text>
</v-card>
...
<div class="section">
<p><span class="merged" id="all.1dEAUE.spl1" title="原文 : To add a ConfigMap as an additional volume to the Pods of a Coherence deployment add entries to the configMapVolumes list in the CRD spec."><code>ConfigMap</code>をCoherenceデプロイメントの<code>Pods</code>に追加ボリュームとして追加するには、CRD仕様の<code>configMapVolumes</code>リストにエントリを追加します。</span> <span class="merged" id="all.1dEAUE.spl2" title="原文 : Each entry in the list has a mandatory name and mountPath field, all other fields are optional.">リストの各エントリには必須<code>name</code>および<code>mountPath</code>フィールドがあり、他のすべてのフィールドはオプションです。</span> <span class="merged" id="all.1dEAUE.spl3" title="原文 : The name field is the name of the ConfigMap to mount and is also used as the volume name."><code>name</code>フィールドは、マウントする<code>ConfigMap</code>の名前で、ボリューム名としても使用されます。</span> <span class="merged" id="all.1dEAUE.spl4" title="原文 : The mountPath field is the path in the container to mount the volume to."><code>mountPath</code>フィールドは、ボリュームをマウントするコンテナのパスです。</span> </p>

<div class="admonition note">
<p class="admonition-inline"><span class="merged" id="all.2UAqjx" title="原文 : Additional volumes added in this way (either ConfigMaps shown here, or Secrets or plain Volumes) will be added to all containers in the Pod.">この方法で追加された追加のボリューム(ここに表示されている<code>ConfigMaps</code>、または<code>Secrets</code>またはプレーン<code>Volumes</code>)は、<code>Pod</code>内のすべてのコンテナに追加されます。</span></p>
</div>
<markup
lang="yaml"
...
<p><span class="merged" id="all.2i98ou" title="原文 : The yaml above would result in a Pod spec similar to the following (a lot of the Pod spec has been omitted to just show the relevant volume information):">前述のyamlは、次のような<code>Pod</code>仕様になります(関連するボリューム情報のみを表示ために、<code>Pod</code>仕様の多くが省略されています):</span></p>
