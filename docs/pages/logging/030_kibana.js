...
<div class="section">
<p><span class="merged" id="all.3Dk67H.spl1" title="原文 : Kibana is often used to anyalze log files that have been collected into Elasticsearch.">Kibanaは、多くの場合、Elasticsearchに収集されているログ・ファイルをオールズするために使用されます。</span> <span class="merged" id="all.3Dk67H.spl2" title="原文 : The Coherence Operator provides a number of Kibana dashboards and queries to allow you to view and analyze logs from your Coherence clusters.">Coherence Operatorには、多数のKibanaダッシュボードおよび問合せがあり、Coherenceクラスタからのログを表示および分析できます。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.1eEZp9" title="原文 : The Kibana dashboard files are located in the Coherence operator source in the dashboards/kibana directory.">Kibanaダッシュボード・ファイルは、<code>dashboards/kibana</code>ディレクトリのCoherence Operatorソースにあります。</span></p>

<p><span class="merged" id="all.1YF3vg.spl1" title="原文 : The method of importing the dashboards into Kibana will depend on how Kibana is being run.">ダッシュボードをKibanaにインポートするメソッドは、Kibanaの実行メソッドによって異なります。</span> <span class="merged" id="all.1YF3vg.spl2" title="原文 : The simplest method is just to import the json file using the Kibana web UI.">最も簡単なメソッドは、Kibana web UIを使用してjsonファイルをインポートすることです。</span> <span class="merged" id="all.1YF3vg.spl3" title="原文 : An alternative approach is to load the dashboard into a ConfigMap in Kubernetes that is mounted into the Kibana Pod and then trigger an import when Kibana starts.">別の方法として、KibanaポッドにマウントされたKubernetesの<code>ConfigMap</code>にダッシュボードをロードし、Kibanaの起動時にインポートをトリガーします。</span> <span class="merged" id="all.1YF3vg.spl4" title="原文 : As there are many ways to do this depending on the specifics of the version of Kibana being used, exact instructions are beyond the scope fo this guide.">使用しているKibanaのバージョンの詳細によっては、これを行う方法が多数あるため、このマニュアルでは説明しません。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.3SKTgU" title="原文 : Information from all dashboards (and queries) can be filtered using the standard Kibana date/time filtering in the top right of the UI, as well as the Add a filter button.">すべてのダッシュボード(および問合せ)からの情報は、UIの右上にある標準のKibana日時フィルタリングおよび<code>Add a filter</code>ボタンを使用してフィルタ処理できます。</span></p>
...
<div class="section">
<p><span class="merged" id="all.2Bk9fz" title="原文 : This dashboard shows Persistence related messages including failed and successful operations.">このダッシュボードには、失敗した操作や成功した操作を含む永続性関連のメッセージが表示されます。</span></p>
...
<div class="section">
<p><span class="merged" id="all.1jYh3G" title="原文 : This dashboard shows configuration related messages such as loading of operational, cache configuration and POF configuration files.">このダッシュボードには、運用可能、キャッシュ構成およびPOF構成ファイルのロードなどの構成関連メッセージが表示されます。</span></p>
...
<div class="section">
<p><span class="merged" id="all.1iEWbL" title="原文 : This dashboard hows network related messages, such as communication delays and TCP ring disconnects.">このダッシュボードには、通信遅延やTCPリング切断などのネットワーク関連のメッセージが表示されます。</span></p>
...
<div class="section">
<p><span class="merged" id="all.3uUjUb.spl1" title="原文 : A number of searches are automatically includes which can help assist in diagnosis and troubleshooting a Coherence cluster.">多数の検索が自動的に組み込まれ、Coherenceクラスタの診断およびトラブルシューティングに役立ちます。</span> <span class="merged" id="all.3uUjUb.spl2" title="原文 : They can be accessed via the Discover side-bar and selecting `Open."><code>Discover</code> <code>side-bar and selecting `Open</code>を介してアクセスできます。</span> </p>
...
<ul class="ulist">
<li>
<p><span class="merged" id="all.yLQ89" title="原文 : Cluster - Cluster join, discovery, heartbeat, member joining and stopping messages">クラスタ - クラスタの結合、検出、ハートビート、メンバーの参加および停止メッセージ</span></p>

</li>
<li>
<p><span class="merged" id="all.1H8LLN" title="原文 : Cache - Cache restarting, exceptions and index exception messages">キャッシュ - キャッシュ再起動、例外、およびインデックス例外メッセージ</span></p>
...
<li>
<p><span class="merged" id="all.3N2GFJ" title="原文 : Network - Network communications delays, disconnects, timeouts and terminations">ネットワーク - ネットワーク通信の遅延、切断、タイムアウト、終了</span></p>

</li>
<li>
<p><span class="merged" id="all.3zUpM1" title="原文 : Partition - Partition loss, ownership and transfer related messages">パーティション - パーティションの損失、所有権、関連メッセージの転送</span></p>

</li>
<li>
<p><span class="merged" id="all.YhTmD" title="原文 : Member - Member thread dump, join and leave messages">メンバー - メンバー・スレッド・ダンプ、メッセージの結合および終了</span></p>
